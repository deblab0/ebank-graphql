...
public class CustomerServiceImpl implements ICustomerService{
    ...
    public AddCustomerResponse createCustomer(AddCustomerRequest addCustomerRequest) {
        ...
        customerRepository.findByIdentityRef(identityRef).ifPresent(a -> {
            throw new BusinessException(String.format("Customer with the same identity [%s] exists", identityRef));
        });
        ...
        response.setMessage(String.format("Customer: [identity=%s,First Name= %s, Last Name = %s, username= %s] was created successfully",
    ...
    public UpdateCustomerResponse updateCustomer(String identityRef, UpdateCustomerRequest updatecustomerRequest) {
        ...
        updateCustomerResponse.setMessage(String.format("Customer identity %s is updated successfully", identityRef));
    ...
    public CustomerDto getCustomByIdentity(String identity) {
        return modelMapper.map(customerRepository.findByIdentityRef(identity).orElseThrow(
                ()-> new BusinessException(String.format("No Customer wit identity %s exist ! ", identity))), CustomerDto.class);
    }

    @Override
    public String deleteCustomerByIdentityRef(String identityRef) {
        if(identityRef == null || identityRef.isEmpty()) throw  new BusinessException(String.format("Enter a correct identity customer"));
